...
#     """Call `agent.run(message)` with retries.

#     Returns the first non-`"none"` answer or the last response after max attempts.
...
#         last_resp = resp
#         if resp.answer and resp.answer != "none":
#             return resp
#         if attempt < max_retries:
#             time.sleep(delay * (2 ** (attempt - 1)))
#     # return the last response (likely with answer == "none")
#     return last_resp


class PhishPondPipeline:
    """Orchestrates multi-model benchmarking over Phish Pond questions."""
    ...
    def execute(self):
        ...
        try:
            processor = PhishPondPreprocessor(self.file_path)
    ...
    def _save_results(self, results_df: pd.DataFrame) -> Path:
        ...
        run_dir = RESULTS_DIR / f"phish_pond_{timestamp}"
