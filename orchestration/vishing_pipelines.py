...
from preprocessing.vishing_preprocessor import VishingPreprocessor
...
def get_agent_response(row, agent: AzureAgent) -> AgentResponse:
    ...
    # retry_count = 0
    # # while (retry_count < 3) and (response.answer in ["none", "undetermined"]):
    # while response.answer in ["none", "undetermined"]:
    #     retry_count+=1
    #     response = agent.run(USER_PROMPT.format(transcript=transcript))
    return response


class VishingPipeline:
    ...
    def execute(self):
        ...
        try:
            processor = VishingPreprocessor(self.file_path)
    ...
    def _save_results(self, results_df: pd.DataFrame) -> Path:
        ...
        run_dir = RESULTS_DIR / f"vishing_{timestamp}"
